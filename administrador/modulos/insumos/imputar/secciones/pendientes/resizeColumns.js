function initColumnResizing() {
    const table = document.querySelector('.imputar-table');
    if (!table) return;
...
function saveColumnWidths() {
    const headers = document.querySelectorAll('.imputar-table th');
    const widths = Array.from(headers).map(th => parseInt(th.style.width) || th.offsetWidth);
    localStorage.setItem('imputar-pendientes-column-widths', JSON.stringify(widths));
}
