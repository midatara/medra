        ...
        traspasarModal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-content-body" id="traspasoBody">
                    <p>¿Desea traspasar <strong>Todos los registros</strong> a <code>imputacion_consigna</code>?</p>
                    <p style="color:#856404;background:#fff3cd;padding:10px;border-radius:4px;font-size:11px;">
                        <strong>Advertencia:</strong> Los registros se eliminarán de <code>ingresos_consigna</code>.
                    </p>
...
async function ejecutarTraspaso() {
    ...
    try {
        const snapshot = await getDocs(collection(db, "ingresos_consigna"));
        if (snapshot.empty) {
        ...
        snapshot.docs.forEach(d => {
            ...
            const newRef = doc(collection(db, "imputacion_consigna"));
            batch.set(newRef, {
                ...data,
                fechaCX: data.fechaCX ? Timestamp.fromDate(new Date(data.fechaCX)) : Timestamp.fromDate(new Date()),
