...
async function loadAllData() {
    showLoading();
    try {
        const q = query(collection(db, 'consigna_historial'), where('estado', '==', 'CARGADO'));
        ...
        if (!availableYears.includes(selectedYear)) {
            selectedYear = availableYears[0] || selectedYear;
        }
...
function groupData(data) {
    ...
    data.forEach(reg => {
        ...
        map.get(key).totalItems += Number(reg.totalItems || 0);
    });
...
function renderTable(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:30px;color:#999;">
            No hay pacientes cargados con los filtros seleccionados
        </td></tr>`;
        return;
    }
